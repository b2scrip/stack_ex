...
with open("eostitle.xml") as f:
    for line in f.readlines():
        ...
        try:
            tree.write("eostmp.xml",encoding="UTF-8")
        except Exception as e:
            print(e)
            print("write eostmp.xml error.........")
        ...
        try:
            _content = Path("eostmp.xml").read_text().replace('<document>"','').replace('" </document>','')
        except Exception as e:
            continue
        ...
        Question.objects.create(postid=_id,ownuserid=_own,title=_trs_title,content=_content,score=_score,viewcount=_viewcount,tags=_tags,answercount=_answercount,creationdate=_date)
        print("succefully...")
